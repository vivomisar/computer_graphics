...
float min(float line[][M], int col){
...
float max(float line[][M], int col){
    float res = line[0][col];
    for (int i = 1; i < M; i++) {
        res = res < line[i][col] ? line[i][col] : res;
    }
...
void fill(float line[][M]){
    float min_y, max_y, y;
    float x[N*2];
    int n;
    min_y = min(line, 1);
    max_y = max(line, 1);
    for(y = min_y+1; y < max_y; y += 0.5){
        n = 0;
        for(int i = 0; i < N; ++i){
            if ((line[i][1] <= y && line[(i+1)%N][1] >= y) || (line[i][1] >= y && line[(i+1)%N][1] <= y)) {
                x[n] = line[i][0] + (y - line[i][1])*(line[(i+1)%N][0] - line[i][0])/(line[(i+1)%N][1] - line[i][1]);
                n++;
            }
        }
        qsort(x, n, sizeof(float), cmp);
        for(int i = 0; i < n; i += 2){
            SDL_RenderDrawLineF(renderer, x[i], y, x[(i+1)%n], y);
        }
    }
...
void update(float line[][M]){
    ...
    SDL_SetRenderDrawColor(renderer, CH_MAX/2, CH_MAX/2, CH_MAX, CH_MAX);
    fill(line);
